...
namespace unityfusionclr
{
	namespace interpreter
	{
		...
		static void* NotSupportInvoke(Il2CppMethodPointer, const MethodInfo* method, void*, void**)
		{
			...
			TEMP_FORMAT(errMsg, "Invoke method missing. ABI:%s sinature:%s %s.%s::%s", UNITYFUSIONCLR_ABI_NAME, sigName, method->klass->namespaze, method->klass->name, method->name);
		...
		{
			if (unityfusionclr::metadata::IsInterpreterImplement(method))
			{
				IL2CPP_ASSERT(method->parameters_count <= 32);
				StackObject newArgs[32];
				int32_t argBaseOffset;
				if (unityfusionclr::metadata::IsInstanceMethod(method))
				{
				...
				unityfusionclr::interpreter::Interpreter::Execute(method, newArgs, ret);
				return;
			}
			if (method->invoker_method == nullptr)
			{
				...
				TEMP_FORMAT(errMsg, "GetManaged2NativeMethodPointer. ABI:%s sinature:%s not support.", UNITYFUSIONCLR_ABI_NAME, sigName);
			...
			for (uint8_t i = 0; i < method->parameters_count; i++)
			{
				...
				if (!argType->byref && unityfusionclr::metadata::IsValueType(argType))
				{
			...
			if (ret)
			{
				...
				if (unityfusionclr::metadata::IsValueType(returnType))
				{
...
#ifdef UNITYFUSIONCLR_UNITY_2021_OR_NEW
	...
	static void InterpreterDelegateInvoke(Il2CppMethodPointer, const MethodInfo* method, void* __this, void** __args, void* __ret)
	{
		...
		for (int32_t i = 0; i < subDelCount; i++)
		{
			...
			case -1:
			{
				IL2CPP_ASSERT(!unityfusionclr::metadata::IsInstanceMethod(curMethod));
			...
			case 1:
			{
				IL2CPP_ASSERT(unityfusionclr::metadata::IsInstanceMethod(curMethod) && curMethod->parameters_count);
	...
	static void* InterpterInvoke(Il2CppMethodPointer methodPointer, const MethodInfo* method, void* __this, void** __args)
	{
		...
		{
			IL2CPP_ASSERT(GetTypeArgDesc(method->return_type).stackObjectSize <= unityfusionclr::metadata::kMaxRetValueTypeStackObjectSize);
			StackObject ret[unityfusionclr::metadata::kMaxRetValueTypeStackObjectSize];
	...
	InterpMethodInfo* InterpreterModule::GetInterpMethodInfo(const MethodInfo* methodInfo)
	{
		...
		metadata::Image* image = metadata::IsInterpreterMethod(methodInfo) ? unityfusionclr::metadata::MetadataModule::GetImage(methodInfo->klass)
			: (metadata::Image*)unityfusionclr::metadata::AOTHomologousImage::FindImageByAssembly(
