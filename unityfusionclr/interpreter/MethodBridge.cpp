	...
	namespace interpreter
	{
		...
		void ConvertInvokeArgs(StackObject* resultArgs, const MethodInfo* method, void** __args)
		{
			for (uint8_t i = 0; i < method->parameters_count; i++)
			{
				...
				else if (unityfusionclr::metadata::IsValueType(argType))
				{
		...
		{
#if UNITYFUSIONCLR_ABI_UNIVERSAL_32 || UNITYFUSIONCLR_ABI_UNIVERSAL_64 || UNITYFUSIONCLR_ABI_WEBGL32
			switch (aligment)
			{
			case 0:
			case 1:
...
#elif UNITYFUSIONCLR_ABI_ARM_64
			if (typeSize <= 8)
			{
...
#elif UNITYFUSIONCLR_ABI_UNIVERSAL_64
			uint8_t actualAligment = 1;
			AppendValueTypeSignatureByAligmentAndSize(typeSize, actualAligment, returnType, sigBuf, bufferSize, pos);
#elif UNITYFUSIONCLR_ABI_UNIVERSAL_32
			uint8_t actualAligment = klass->minimumAlignment <= 4 ? 1 : 8;
			AppendValueTypeSignatureByAligmentAndSize(typeSize, actualAligment, returnType, sigBuf, bufferSize, pos);
#elif UNITYFUSIONCLR_ABI_WEBGL32
...
#if UNITYFUSIONCLR_ABI_ARM_64 || UNITYFUSIONCLR_ABI_UNIVERSAL_64 || UNITYFUSIONCLR_ABI_UNIVERSAL_32 || UNITYFUSIONCLR_ABI_WEBGL32
...
#error "not suppport platform"
		...
		bool ComputeSignature(const Il2CppMethodDefinition* method, bool call, char* sigBuf, size_t bufferSize)
		{
			...
			const Il2CppImage* image = unityfusionclr::metadata::MetadataModule::GetImage(method)->GetIl2CppImage();

			AppendSignature(unityfusionclr::metadata::MetadataModule::GetIl2CppTypeFromEncodeIndex(method->returnType), true, sigBuf, bufferSize, pos);
			...
			{
				TypeIndex paramTypeIndex = unityfusionclr::metadata::MetadataModule::GetParameterDefinitionFromIndex(image, method->parameterStart + i)->typeIndex;
				AppendSignature(unityfusionclr::metadata::MetadataModule::GetIl2CppTypeFromEncodeIndex(paramTypeIndex), false, sigBuf, bufferSize, pos);
			}
			sigBuf[pos] = 0;
			return true;
		}
