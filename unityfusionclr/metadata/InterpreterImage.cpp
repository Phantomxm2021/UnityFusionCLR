...
namespace metadata
{
	...
	void InterpreterImage::ComputeBlittable(Il2CppTypeDefinition* def, std::vector<bool>& computFlags)
	{
		...
		if (type->type == IL2CPP_TYPE_VALUETYPE)
		{
			...
			for (int i = 0; i < def->field_count; i++)
			{
				...
				if (!unityfusionclr::metadata::IsInstanceField(fieldType))
				{
					continue;
				}
...
#endif
#if !UNITYFUSIONCLR_UNITY_2022_OR_NEW
	...
	{
		unityfusionclr::interpreter::ExecutingInterpImageScope scope(unityfusionclr::interpreter::InterpreterModule::GetCurrentThreadMachineState(), _il2cppImage);
	...
	CustomAttributesCache* InterpreterImage::GenerateCustomAttributesCacheInternal(CustomAttributeIndex index)
	{
		...
		unityfusionclr::interpreter::ExecutingInterpImageScope scope(unityfusionclr::interpreter::InterpreterModule::GetCurrentThreadMachineState(), this->_il2cppImage);
...
#elif UNITYFUSIONCLR_UNITY_2021
	CustomAttributesCache* InterpreterImage::GenerateCustomAttributesCacheInternal(CustomAttributeIndex index)
	{
		IL2CPP_ASSERT(index != kCustomAttributeIndexInvalid);
		CustomAttributesCache* cache = _customAttribtesCaches[index];
	...
	Il2CppMethodPointer InterpreterImage::GetAdjustorThunk(uint32_t token)
	{
		...
		return IsInstanceMethod(methodDef) ? unityfusionclr::interpreter::InterpreterModule::GetAdjustThunkMethodPointer(methodDef) : nullptr;
	}
	...
	Il2CppMethodPointer InterpreterImage::GetMethodPointer(uint32_t token)
	{
		...
		IL2CPP_ASSERT(methodIndex < (uint32_t)_methodDefines.size());
		const Il2CppMethodDefinition* methodDef = &_methodDefines[methodIndex];
		return unityfusionclr::interpreter::InterpreterModule::GetMethodPointer(methodDef);
	}
	...
	InvokerMethod InterpreterImage::GetMethodInvoker(uint32_t token)
	{
		...
		IL2CPP_ASSERT(methodIndex < (uint32_t)_methodDefines.size());
		const Il2CppMethodDefinition* methodDef = &_methodDefines[methodIndex];
		return unityfusionclr::interpreter::InterpreterModule::GetMethodInvoker(methodDef);
	}
