...
using namespace unityfusionclr::metadata;
using namespace unityfusionclr::interpreter;
...
#pragma endregion
		...
		LabelCall:
			{
				...
				if (unityfusionclr::metadata::IsInterpreterImplement(shareMethod))
				{
		...
		LabelCallVir:
			{
				IL2CPP_ASSERT(shareMethod);
				IL2CPP_ASSERT(unityfusionclr::metadata::IsInstanceMethod(shareMethod));
			...
			case OpcodeValue::LDSFLDA:
			{
				...
				if (fieldInfo->offset != THREAD_STATIC_FIELD_OFFSET)
				{
					...
					if (unityfusionclr::metadata::IsInterpreterType(fieldInfo->parent))
					{
						const FieldDetail& fieldDet = unityfusionclr::metadata::MetadataModule::GetImage(fieldInfo->parent)
							->GetFieldDetailFromRawIndex(unityfusionclr::metadata::DecodeTokenRowIndex(fieldInfo->token - 1));
